...
for (increments_number in 2:num_model_increments){
  ...
  for (i in 1:16){ #shift all other age classes up
    next_state[i,PCV_start_bracket[1]+1]=prev_state[i,PCV_start_bracket[1]]
  }
  for (i in 1:2){ #for normal classes and then maternally immunised
    for (j in 1:2){ #for S then I
  ...
  #apply third dose
  for (i in 1:16){ #shift all other age classes up
    next_state[i,PCV_start_bracket[2]+1]=prev_state[i,PCV_start_bracket[2]]
  }
  for (i in 1:2){ #for normal classes and then maternally immunised
    for (j in 1:2){ #for S then I
  ...
  next_state[,num_age_groups-1]=prev_state[,num_age_groups-2]+(17/18)*prev_state[,num_age_groups-1]
  next_state[,num_age_groups]=(1/18)*prev_state[,num_age_groups-1]+prev_state[,num_age_groups]
  
  #waning maternal immunity as an exponential ###################################
  #COMEBACK 
  for (i in 1:2){ #S and I
    for (j in 1:4){ #S,V1,V2,V3
      for (k in 1:8) { #age groups
      ...
      for (k in 9){ #cut off maternal immunity after 12 months
  ...
  # next_state[,14]=next_state[,14]-mu[14]*(next_state[,14]/total)
  # 
  S_next=next_state[1,]
