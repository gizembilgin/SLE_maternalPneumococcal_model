...
mat_length = 6  # length of maternal immunity in months(must be divisible by 2) #COMEBACK
...
mod_fac2 = 1  # modification on transmission from carriage
...
####################################################################
#COMEBACK these are all deterministic, could be replaced with probabilitistic
pcv_effectiveness = "standard"  # choose between "standard", "lower" and "higher"
pcv_coverage = "standard"  # choose between "standard", "lower" and "higher"
...
N = 6875000           # total pop size
life_expectancy = 52  # life expectancy
pop_distribution = "standard"  # choose between "standard" and "census"

# importing carriage, recovery, tranmission probability and contact 
#COMEBACK
...
recovery=recov_adj*recovery;
carriage=carriage_adj*carriage;


contact_matrix <- read.csv("contact_matrix.csv",header=TRUE)
tranmission_probab <- read.csv("transmission_matrix.csv",header=TRUE)
#####################################################################
...
# maternal vaccine parameters
if (maternal_vaccine_on == "Y"){
  meff = 0.5  # maternal vaccine effectiveness
  mcov = 0.95  # maternal vaccine coverage, currently assumed to be equal to first dose of PCV
...
if (mat_waning == "standard"){
  wan = 0.41  # standard fit to average of all serotypes waning
  } else if (mat_waning == "slower"){
  wan = 0.36  # slower waning
  } else if (mat_waning == "faster"){
  wan = 0.575  # faster waning
  }else{
...
if (pcv_effectiveness == "standard"){
  e = c(0.24, 0.5, 0.58)
  } else if (pcv_effectiveness == "lower"){
  e = c(0.17, 0.35, 0.4)
  } else if (pcv_effectiveness == "higher"){
  e = c(0.3, 0.65, 0.75)
  }else{
  warning("pcv effectiveness not selected, assuming standard pcv_effectiveness")
  e = c(0.24, 0.5, 0.58)
  }

# pcv coverage of doses
if (pcv_coverage == "standard"){
  true_cov = c(92.33, 79.05, 57.26)  # from MICS 2017 UNICEF data-set
  }else if (pcv_coverage == "lower"){
  true_cov = c(60, 52, 37)
  }else if (pcv_coverage == "higher"){
  true_cov = c(98, 84, 61)
  }else{
  warning("no pcv coverage selected, assuming standard pcv_coverage")
  true_cov = c(0.24, 0.5, 0.58)
}
...
contact_transmission=contact_matrix[-1]*tranmission_probab[-1]
...
if (pop_distribution == "standard"){ #uniform distribution according to standard life expectancy
  for (i in 1:12){P_inital[i]=N/(life_expectancy*6)}
  P_inital[13]=N*3/life_expectancy
  P_inital[14]=N*(life_expectancy-5)/life_expectancy
} else if (pop_distribution=="census") {
  for (i in 1:6){P_inital[i]=(0.02463*N)/6} # guess all 0-2,...10-12 month groups equal size; should be 2.463%
  for (i in 7:12){P_inital[i]=(0.02692*N)/6} # guess all 12-14,...,22-24 etc. age groups equal size; should be 2.692%
  P_inital[13]=N*0.11218 # 2-5 years
  P_inital[14]=N*0.8364 #5+ age group
}

nu=P_inital[1]   #set birth rate (per 2 months) to inital number in 0-2 months
mu=nu            #death rate set to birth rate
...
pneumODE <- function(t, state, parameters){
  ...
  with(as.list(c(state,parameters)),{
    ...
    dV1m[i]=-trans[i]*V1m[i]*(1-e[1]*(1-blunting))*(1-meff)+recovery[i]*Iv1m[i]
    dV2m[i]=-trans[i]*V2m[i]*(1-e[2]*(1-blunting))*(1-meff)+recovery[i]*Iv2m[i]
    dV3m[i]=-trans[i]*V3m[i]*(1-e[3]*(1-blunting))*(1-meff)+recovery[i]*Iv3m[i]
    dIsm[i]=trans[i]*Sm[i]*(1-meff)-recovery[i]*Ism[i]
    dIv1m[i]=trans[i]*V1m[i]*(1-e[1]*(1-blunting))*(1-meff)-recovery[i]*Iv1m[i]
    dIv2m[i]=trans[i]*V2m[i]*(1-e[2]*(1-blunting))*(1-meff)-recovery[i]*Iv2m[i]
    dIv3m[i]=trans[i]*V3m[i]*(1-e[3]*(1-blunting))*(1-meff)-recovery[i]*Iv3m [i]
    
    dIncidence[i]=trans[i]*(S[i]+Sm[i]*(1-meff)+V1[i]*(1-e[1])+V1m[i]*(1-meff)*(1-e[1]*(1-blunting))+
      V2[i]*(1-e[2])+V2m[i]*(1-meff)*(1-e[2]*(1-blunting))+V3[i]*(1-e[3])+V3m[i]*(1-meff)*(1-e[3]*(1-blunting)))
